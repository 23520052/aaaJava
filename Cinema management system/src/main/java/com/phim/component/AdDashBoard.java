...
public class AdDashBoard extends javax.swing.JPanel {
    ...
    private ChiTietDatChoController chitietdatchoController = new ChiTietDatChoController();
    ...
    private JPanel topPhimPanel;
    private JPanel topNhanVienPanel;
    private JPanel nhanvienRolePanel;
    ...
    private void setupDashboard() {
        ...
        statsRow.add(nhanvienPanel);
        statsRow.add(phimPanel);
        statsRow.add(thanhtoanPanel);
        statsRow.add(doanhthuPanel);
        ...
        revenueChartContainer.add(createPanelWithBorder(doanhthuChartPanel, "Biểu đồ doanh thu"), BorderLayout.CENTER);
        ...
        // 6. Top phim chart
        JPanel topphimContainer = new JPanel(new BorderLayout());
        topphimContainer.setBackground(BACKGROUND_COLOR);
        topphimContainer.add(createPanelWithBorder(topPhimPanel, "Phim có lượt mua cao"), BorderLayout.CENTER);
        topphimContainer.setAlignmentX(Component.LEFT_ALIGNMENT);
        panelStatistic.add(topphimContainer);
        panelStatistic.add(Box.createVerticalStrut(15));
        ...
        topEmployeesContainer.add(createPanelWithBorder(topNhanVienPanel, "Top nhân viên"), BorderLayout.CENTER);
        ...
        roleContainer.add(createPanelWithBorder(nhanvienRolePanel, "Vai trò nhân viên"), BorderLayout.CENTER);
    ...
    private void createStatisticsPanels() {
        nhanvienPanel = createStatPanel(
            "Nhân viên",
            String.valueOf(nhanvienController.countNhanVien()),
            "Tổng số nhân viên",
            PRIMARY_COLOR
        );

        phimPanel = createStatPanel(
            "Phim",
            String.valueOf(phimController.countPhim()),
            "Tổng số phim",
            SECONDARY_COLOR
        );

        thanhtoanPanel = createStatPanel(
            "Hóa đơn",
            String.valueOf(thanhtoanController.getSoLuongThanhToan()),
            "Tổng số hóa đơn",
            ACCENT_COLOR
        );

        NumberFormat formatter = NumberFormat.getCurrencyInstance(new Locale("vi", "VN"));
        String totalRevenue = formatter.format(thanhtoanController.getTongDoanhThu());
        doanhthuPanel = createStatPanel(
            "Doanh thu",
            totalRevenue,
            "Tổng doanh thu",
    ...
    private void updateYearCombo() {
        ...
        List<Integer> years = thanhtoanController.getAvailableYears();
    ...
    private JPanel createDailyRevenueWidget() {
        ...
        NumberFormat formatter = NumberFormat.getCurrencyInstance(new Locale("vi", "VN"));
        String dailyRevenue = formatter.format(thanhtoanController.getTongTienTheoNgay());
        JPanel contentPanel = new JPanel();
        contentPanel.setLayout(new BoxLayout(contentPanel, BoxLayout.Y_AXIS));
        contentPanel.setBackground(CARD_COLOR);
        JPanel colorStrip = new JPanel();
    ...
    private JPanel createMonthlyRevenueWidget() {
        ...
        NumberFormat formatter = NumberFormat.getCurrencyInstance(new Locale("vi", "VN"));
        String monthlyRevenue = formatter.format(thanhtoanController.getTongTienTheoThangNam());
        JPanel contentPanel = new JPanel();
        contentPanel.setLayout(new BoxLayout(contentPanel, BoxLayout.Y_AXIS));
        contentPanel.setBackground(CARD_COLOR);
        JPanel colorStrip = new JPanel();
    ...
    private void updateCharts() {
        ...
        switch (selectedRange) {
            case "Theo năm":
                List<ChartDataModel> yearlyData = thanhtoanController.getYearlyRevenue();
                ...
                break;
            case "Theo tháng":
                List<ChartDataModel> monthlyData = thanhtoanController.getMonthlyRevenue(selectedYear);
                ...
                break;
            case "Theo tuần":
                List<ChartDataModel> weeklyData = thanhtoanController.getWeeklyRevenueByYear(selectedYear);
        ...
        doanhthuChartPanel.removeAll();
        ChartPanel chartPanel = new ChartPanel(chart);
        chartPanel.setPreferredSize(new Dimension(400, 300));
        doanhthuChartPanel.add(chartPanel);
        doanhthuChartPanel.revalidate();
        doanhthuChartPanel.repaint();
    }

    private void createCharts() {
        // Revenue chart
        doanhthuChartPanel = new JPanel(new BorderLayout());
        doanhthuChartPanel.setBackground(Color.WHITE);
        ...
        doanhthuChartPanel.add(revenueCP);

        // Top Phim chart
        topPhimPanel = new JPanel(new BorderLayout());
        topPhimPanel.setBackground(Color.WHITE);

        DefaultCategoryDataset phimDataset = new DefaultCategoryDataset();
        List<Map<String, Object>> topPhim = phimController.getTopSellingPhim(5);
        for (Map<String, Object> item : topPhim) {
            phimDataset.addValue((Integer) item.get("quantity"), "Số lượng", item.get("TEN_PHIM").toString());
        }

        JFreeChart phimChart = ChartFactory.createBarChart(
            "Top phim bán chạy", "Phim", "Số lượng", phimDataset,
            PlotOrientation.VERTICAL, false, true, false
        );
        phimChart.setBackgroundPaint(Color.WHITE);
        phimChart.getTitle().setFont(new Font("Segoe UI", Font.BOLD, 14));
        phimChart.getTitle().setPaint(PRIMARY_COLOR);

        CategoryPlot phimPlot = phimChart.getCategoryPlot();
        phimPlot.setBackgroundPaint(Color.WHITE);
        phimPlot.setDomainGridlinePaint(new Color(220, 220, 220));
        phimPlot.setRangeGridlinePaint(new Color(220, 220, 220));
        BarRenderer phimRenderer = (BarRenderer) phimPlot.getRenderer();
        phimRenderer.setSeriesPaint(0, SECONDARY_COLOR);
        phimRenderer.setBarPainter(new org.jfree.chart.renderer.category.StandardBarPainter());
        phimRenderer.setShadowVisible(false);

        ChartPanel phimCP = new ChartPanel(phimChart);
        phimCP.setPreferredSize(new Dimension(400, 300));
        topPhimPanel.add(phimCP);

        // Top Employees chart
        topNhanVienPanel = new JPanel(new BorderLayout());
        topNhanVienPanel.setBackground(Color.WHITE);

        DefaultCategoryDataset employeesDataset = new DefaultCategoryDataset();
        // Lấy List<Map<String,Object>> từ DAO qua Controller
        List<Map<String, Object>> topEmployees = thanhtoanController.getTopEmployeesByRevenue(5);
        for (Map<String, Object> emp : topEmployees) {
            // Trong DAO chúng ta đã map keys: "MaNV", "FullName", "DOANHTHU"
            int MaNV           = (Integer)    emp.get("MaNV");
            String fullName          = (String)     emp.get("FullName");
            BigDecimal totalRevenueBD= (BigDecimal) emp.get("DOANHTHU");
            double revenueValue      = totalRevenueBD.doubleValue();

            String label = fullName + " (" + MaNV + ")";
        ...
        topNhanVienPanel.add(employeesCP);

        // Employee Role distribution
        nhanvienRolePanel = new JPanel(new BorderLayout());
        nhanvienRolePanel.setBackground(Color.WHITE);

        org.jfree.data.general.DefaultPieDataset roleDataset = new org.jfree.data.general.DefaultPieDataset();
        List<ChartDataModel> roleData = nhanvienController.getEmployeeCountByRole();
